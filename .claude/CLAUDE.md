...
当前顺序：Task A 只维护；GA4 唯一实验、GSC 7/28 实时 9 URL 分诊以及 Shopping/Enhancements 下降诊断已完成。下一源码窗口先修 Review/Product schema 语义合规（不得复用 Amazon 第三方评分或伪造 Merchant 政策），再按日期执行 7/29 的 30 天 Sprint 分轨裁决与后续 Feeder Reliability coverage-gap。Pretty Happy Pets 跟进已 `Sent-confirmed`，不得重复发送；Round 4 剩余三封继续 Hold。
