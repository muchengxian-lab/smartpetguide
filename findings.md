...
**口径微调**：7/28 作为 Rich Results 口径断点。Product/Merchant/Review 后续数量不与撤标前直接同比，也不据此判断普通索引、排名或流量变化；周五必须分别记录 Page Indexing、query/page、重定向验证、5 个精准重抓状态和 Rich Results。

**Week 13 决策闸门**：8/2 普通索引 ≥30 时继续原战略；25-29 时观察并保持新 URL 为 0；<25 或核心价值页掉出索引时，才启用 Month 2 已有的索引应急规则。5 个 URL 已请求重抓，本周不重复或扩大 Inspection。
...
**发现**：周期起点索引为 22，7/15 已升至 33并保持；最近 7 天只有 9 次展示、0 点击，但平均排名从前 7 天的 65.6 改善到 51.1。索引目标提前完成，当前瓶颈不再是“让 Google 发现更多 URL”，而是已索引页面的查询覆盖、排名、CTR 和外部权威。

**决策**：Week 11 不再机械执行每日 URL Inspection ×5。4 个已抓取未索引页面只在实质改动、状态变化或验证到期时提交；页面工作优先 `cat-wont-drink-from-water-fountain`，并观察 `litter-robot-5-vs-litter-robot-4` 与 `introduce-cat-to-automatic-litter-box` 的新展示。

### Pretty Happy Pets 已进入正式审稿与双向编辑评估

**发现**：7/19 对方确认编辑与兽医团队已开始审核客稿，如需修改会直接在 Google Doc 留言；同时承诺下周提供 2-3 个拟投 SmartPetGuide 的主题和写作样稿。

**决策**：现在应在原邮件线程简短确认一次，随后等待评论与提案，不询问发布时间。对方来稿按相关性、原创性、来源质量和 SmartPetGuide 方法论独立审核；不承诺 reciprocal/dofollow。若 7/26 前完全无更新，最早 7/27 轻量跟进一次。当前状态仍不是 `Published`、`Placed` 或 `Backlink won`。

### Round 4 改为两封首批，而不是一次铺开五封
...
**决策**：SmartPetGuide 仓库只保存问题清单和验收标准，具体修复由知识库主控对话在 `E:\knowledge-base` 与自动化脚本仓库执行。交接文件为 `docs/ai-signal-automation-optimization-handoff-2026-07-19.md`；当前项目计划只消费修正后的输出，不在本站任务中重复修自动化。

## Week 10 收盘决策（2026-07-17）

### 索引桶位改善，但不是“已发现未索引”清零突破

**发现**：7/13 的真实基线是“已发现-未索引 0、已抓取-未索引 8”，不是“已发现-未索引 8”。到 7/15 与 7/17，已发现-未索引仍为 0，已抓取-未索引降至 4；同期已索引从 22 增至 33 后持平。现有数据只能证明桶位和索引数量改善，不能把变化单独归因于内链、URL Inspection 或自然重抓。

**决策**：页面发现当前不是第一瓶颈，但仍有 4 个已抓取-未索引页面需要观察。Week 11 将主要精力转向已索引页面的曝光和点击；4 页只在实质改动、状态变化或验证到期时提交，不再使用“索引管道已完全打通”的表述。

### GA4 首次出现美国用户和引荐流量

**发现**：GA4 过去 7 天有 7 个美国活跃用户，较 7/13 的 6 个增加 1，并非首次出现。会话获取为 Direct 29、Referral 2、Organic Search 0；首次用户来源/媒介则为 `(direct) / (none)` 30、占 100%。这两组指标口径不同。

**决策**：美国用户和引荐流量出现是正向信号但样本太小（7 人/7 天），尚不能判定为自然搜索增长或算法测试。继续每周追踪来源分布，关注 Organic Search 何时破零。

### Pinterest 月浏览稳定在 1,400 左右

**发现**：Pinterest 月浏览 1,391（vs Snapshot 5 的 1,423，-2%），基本稳定。Pinterest Analytics 页面本次未正常渲染，出站点击尚未确认，不能记为 0。

**决策**：维持「只观察，不铺量」策略。出站点击完成后台核验前不恢复 Pin 发布配额。当前 5 Board/62+ Pin 的历史资产规模已足以继续观察渠道方向。

### Vercel 直连确认

**发现**：HTTP 响应头确认 Server: Vercel，无 Cloudflare（无 cf-ray/cf-cache-status）。此前的 "crawler/Cloudflare 核对" 任务基于错误假设。

**决策**：crawler 规则完全由 robots.txt + Vercel headers 控制，当前 12 个 AI 爬虫全部 Allow，无已知漏洞。此项从持续任务列表中移除，纳入月度 GEO 复检的 5 分钟存活检查。

### Week 11 方向：Continue

**决策**：维持 Week 10 的权重分配（外链 35-40% / GSC 25-30% / GEO 20-25% / Pinterest 维护），不做大幅调整。理由：（1）索引数量和已抓取未索引桶位刚出现改善，需要 2-4 周观察是否转化为曝光和点击；（2）编辑型外链从接受到落链需要时间；（3）Brand Outreach 样本过小，需等 Homerunpet v2 结果；（4）Month 2 战略 4 周周期刚过 1/4。

### Aorkuler 验证了交付质量，但没有验证付费实施需求

**发现**：John 在 7/17 明确认可 visibility snapshot 的研究与分析质量，但 Aorkuler 当前已有 Amazon 和 DTC 策略在执行，希望先完成既有计划，因此现阶段不实施本次建议。这证明“数据洞察邮件 → 免费快照”能够建立信任并完成交付，不代表品牌当前存在预算、优先级或实施意愿。

**决策**：Aorkuler 标记为 `Closed — not now (positive)`，7/17 礼貌回信后不再主动跟进；最早 60-90 天后只在出现实质触发条件时重新联系。该邮件是同一品牌机会的收口，不新增 Brand 回复数，也不触发 PETKIT/Catit 扩量。Week 11 继续等待 Homerunpet v2 结果，并把主要增长资源留在编辑型外链、可引用资产与现有页分发。

### 本周期末未闭环项

1. GSC 4 个"已抓取-尚未编入索引"页等待重抓；Week 11 只在实质改动、状态变化或验证到期时提交，不做机械轮换。
2. Pinterest 出站点击数据需人工确认。
3. GEO Score 月度复检建议 8/1 执行。
4. Guest Post Round 4 五目标 Researched，待 Week 11 草拟和发送。
...
**重定向错误 4**：GSC 明细为 WOPET 无尾斜杠旧 URL + 3 个历史 URL。当前 HTTP 已是 3 条直接 200、WOPET 单跳 308 到带尾斜杠 200；源码、llms、RSS、sitemap 与生成 HTML 均只引用规范 URL。生产部署复核后已启动一次新的整桶验证，当前为待定 4 / 失败 0；后续等待 Google 重抓并重新分类，不重复点击。
