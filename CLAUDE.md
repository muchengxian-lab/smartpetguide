...
3. Week 13 Tuesday is complete: the feeder audit retained one candidate angle—calibrate the feeder before interpreting food-seeking behavior—but the asset is HOLD because the page links a WOPET PDF that says only about 10 g per portion. A separate verified official WOPET F01 Plus support page does support the 5 g / 10 g wheel distinction. On 8/5, use the one-page factual-consistency allowance to replace and verify that citation first; any sitter/failover finding then remains read-only so a second page is not changed.
